...
/**
 * Returns an approximate reading time based on number of words
 */
function lightspeed_reading_time() {
    
    $content = get_the_content('','',NULL);
    $time = round(str_word_count($content)/200); //avg people read 200 words per min.
    
    if ($time >= 1)
        echo "<span class='reading-time'>".$time." min read</span>";
    else 
        echo "<span class='reading-time'>Quick read</span>";
